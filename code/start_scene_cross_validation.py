...
def start_scene_cross_validation(dataset_name,video_event_name,video_rgb_name,config_name, exception_scenes = []):
    ...
    for scene in scenes:
        ...
        if id is None:
            id = -1
        id = subprocess.check_output(['bash', "create_scene_split.sh", dataset_name, scene,video_event_name,video_rgb_name,config_name,str(id),is_dry]).decode('utf-8').strip().split("\n")[-1]
...
if __name__ == "__main__":
    ...
    parser.add_argument("-dry",action="store_true",help="Dry run: Only creating the sbatch scripts not submitting them to the cluster")
    ...
    if args.dry:
        ...
        is_dry = 0
    start_scene_cross_validation(args.dataset,args.video_event_name,args.video_rgb_name,args.config_name,args.exception_scenes,is_dry)
